...
INT) PRO : BEGIN 
	...
	from bank_card
	where
	    b_number = dest_card_id;
	if s_id != applicant_id
	or r_id != receiver_id
	or (
	    s_type = "信用卡"
	    and r_type = "储蓄卡"
	...
	if s_type = "信用卡" then set amount = -amount;
	end if;
	if r_type = "信用卡" then set rcv_amount = -rcv_amount;
	end if;
	update bank_card
	set
	...
	update bank_card
	set
	    b_balance = b_balance + rcv_amount
