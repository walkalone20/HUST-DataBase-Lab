...
CREATE TABLE
    IF NOT EXISTS `rbac`.`aprole` (
        ...
        PRIMARY KEY (`RoleNo`)
    ) ENGINE = InnoDB DEFAULT CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '角色表';
...
CREATE TABLE
    ...
    ) ENGINE = InnoDB DEFAULT CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '角色分配表';
...
CREATE TABLE
    IF NOT EXISTS `rbac`.`apmodule` (
        ...
        PRIMARY KEY (`ModNo`)
    ) ENGINE = InnoDB DEFAULT CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '功能模块登记表';
...
CREATE TABLE
    IF NOT EXISTS `rbac`.`apright` (
        `RoleNo` INT NOT NULL COMMENT '角色编号',
        `ModNo` BIGINT NOT NULL COMMENT '模块编号',
        PRIMARY KEY (`RoleNo`, `ModNo`),
        INDEX `FK_apRight_apModule` (`ModNo` ASC) VISIBLE,
        CONSTRAINT `FK_apRight_apModule` FOREIGN KEY (`ModNo`) REFERENCES `rbac`.`apmodule` (`ModNo`),
        CONSTRAINT `FK_apRight_apRole` FOREIGN KEY (`RoleNo`) REFERENCES `rbac`.`aprole` (`RoleNo`)
    ) ENGINE = InnoDB DEFAULT CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '角色权限表';
